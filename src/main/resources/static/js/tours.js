  ...
  <div class="col">
    <div class="card h-100 bg-secondary-subtle">
      <img style="width:auto; height:300px;" src="${product.image}" class="card-img-top" alt="${product.title}" 
        ...
        data-product-price="${product.price}" data-product-category="${product.category}" data-product-image="${product.image}">
      <div class="card-body">
        ...
        class="btn btn-dark card-img-top">View</button>
      </div>
    </div>
  </div>`;
...
document.addEventListener("click", function (event) {
  if (event.target && event.target.matches(".card-img-top")) {
    ...
    const productImageSrc = event.target.getAttribute("data-product-image");
